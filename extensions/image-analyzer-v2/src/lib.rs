                    ...
                    if path.is_dir() {
                        ...
                        if let Ok(files) = std::fs::read_dir(&path) {
                            for file in files.flatten() {
                                ...
                                // Extract unversioned library name
                                let unversioned = if cfg!(target_os = "macos") {
                                    ...
                                    // Linux: libfoo.so.1 -> libfoo.so, libfoo.so.1.2.3 -> libfoo.so
                                    if let Some(idx) = name.find(".so.") {
                                        let base = &name[..idx]; // e.g. "libonnxruntime"
                                        Some(format!("{}.so", base))
                                    } else if name.contains('.') && name.ends_with(".dylib") {
                                        // Handle macOS dylib on Linux (unlikely but safe)
                                        None
                                    } else {
                                        None
                                    }
