//! NeoMind MOSS-TTS-Nano Extension
//!
//! Voice-cloning TTS via a Python FastAPI service wrapping
//! `OnnxTtsRuntime` from the MOSS-TTS-Nano ONNX CPU backend.
//!
//! # Commands
//!
...
const DEFAULT_SERVICE_URL: &str = "http://127.0.0.1:9382";
...
pub struct MossTtsExtension {
    inner: Arc<Inner>,
}

impl MossTtsExtension {
    pub fn new() -> Self {
        let service_url = std::env::var("MOSS_TTS_SERVICE_URL")
            .unwrap_or_else(|_| DEFAULT_SERVICE_URL.to_string());
        let voice = std::env::var("MOSS_TTS_VOICE")
...
impl Default for MossTtsExtension {
    fn default() -> Self {
        Self::new()
    }
}
...
fn audio_thread(rx: Receiver<AudioCmd>) {
    ...
    let (_stream, handle): (OutputStream, OutputStreamHandle) = match rodio_open_output_stream() {
        Ok(v) => v,
        Err(e) => {
            eprintln!("[moss-tts] open audio device failed: {e}");
...
#[async_trait]
impl Extension for MossTtsExtension {
    fn metadata(&self) -> &ExtensionMetadata {
        static META: OnceLock<ExtensionMetadata> = OnceLock::new();
        META.get_or_init(|| {
            ExtensionMetadata::new("moss-tts-nano", "MOSS-TTS-Nano", "2.7.6")
                .with_description(
                    "Multilingual 0.1B voice-cloning TTS via MOSS-TTS-Nano (ONNX CPU). \
    ...
    fn commands(&self) -> Vec<ExtensionCommand> {
        vec![
            CommandBuilder::new("synthesize")
                ...
                .description("Synthesize speech via MOSS-TTS-Nano and return WAV as base64. Does NOT play on host audio.")
                .param(
                ...
                .sample(json!({"text":"你好，世界","voice":"Junhao"}))
                .build(),
            CommandBuilder::new("speak")
                ...
                .sample(json!({"text":"你好，世界","voice":"Junhao","blocking":true}))
                .build(),
...
// Built-in voice presets. Mirror of MOSS-TTS-Nano `runtime.list_builtin_voices()`
// as of 2025-06. If upstream adds voices, run `list_voices` and update this list.
...
fn voice_param() -> ParameterDefinition {
    ParamBuilder::new("voice", MetricDataType::String)
        .display_name("Voice")
        .description("Built-in voice preset. Overrides `MOSS_TTS_VOICE` default.")
...
neomind_extension_sdk::neomind_export!(MossTtsExtension);
...
mod tests {
    ...
    #[test]
    fn metadata_is_sane() {
        let ext = MossTtsExtension::new();
        let m = ext.metadata();
        assert_eq!(m.id, "moss-tts-nano");
