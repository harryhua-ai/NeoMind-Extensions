//! NeoMind CosyVoice 3 Extension
//!
//! Streaming TTS via a Python FastAPI service wrapping
//! `FunAudioLLM/Fun-CosyVoice3-0.5B-2512` (24 kHz mono). Targets ~150ms
//! first-chunk latency and <500ms per-sentence synthesis.
//!
//! # Commands
//!
...
//! - `list_voices`  — list voice presets from the service.
//! - `health`       — ping the Python service.
//!
//! # Architecture
//!
//! Adapter layer: the Python service (`service/server.py`) wraps CosyVoice 3
//! and exposes the **same `/tts/stream` NDJSON contract** as moss-tts-nano,
//! so voice-assistant can switch backends by changing one env var. The Rust
//! extension is a thin HTTP proxy identical in shape to `moss-tts-nano`.
...
const DEFAULT_SERVICE_URL: &str = "http://127.0.0.1:9385";
const DEFAULT_VOICE: &str = "中文女";
const DEFAULT_SAMPLE_MODE: &str = "greedy";
// CosyVoice 3 outputs 24 kHz mono int16 PCM.
...
pub struct CosyVoiceExtension {
    inner: Arc<Inner>,
}

impl CosyVoiceExtension {
    pub fn new() -> Self {
        let service_url = std::env::var("COSYVOICE_SERVICE_URL")
            .or_else(|_| std::env::var("MOSS_TTS_SERVICE_URL"))
            .unwrap_or_else(|_| DEFAULT_SERVICE_URL.to_string());
        let voice = std::env::var("COSYVOICE_VOICE")
            .or_else(|_| std::env::var("MOSS_TTS_VOICE"))
...
impl Default for CosyVoiceExtension {
    fn default() -> Self {
        Self::new()
    }
}
...
impl Inner {
        ...
        F: FnMut(&[i16], u32, u16),
    {
        ...
        for line in reader.lines() {
            ...
            // Error sentinel (mirrors moss-tts-nano protocol).
...
// ============================================================================
// Audio playback thread (identical to moss-tts-nano — single rodio stream)
// ============================================================================
...
fn audio_thread(rx: Receiver<AudioCmd>) {
    ...
    let (_stream, handle): (OutputStream, OutputStreamHandle) = match rodio_open_output_stream() {
        Ok(v) => v,
        Err(e) => {
            eprintln!("[cosyvoice-3] open audio device failed: {e}");
...
impl Inner {
    ...
    fn speak_blocking(
        ...
        if wait_for_completion {
            ...
            // sample_rate / channels come from the stream; approximate
            // duration using defaults when service didn't override.
            Ok(json!({
                "played": res.played,
                "finished": true,
                "frames": frames,
...
#[async_trait]
impl Extension for CosyVoiceExtension {
    fn metadata(&self) -> &ExtensionMetadata {
        static META: OnceLock<ExtensionMetadata> = OnceLock::new();
        META.get_or_init(|| {
            ExtensionMetadata::new("cosyvoice-3", "CosyVoice 3", "2.7.6")
                .with_description(
                    "Streaming 0.5B TTS via Fun-CosyVoice3-0.5B-2512 (24kHz mono, ~150ms TTFA). \
    ...
    fn commands(&self) -> Vec<ExtensionCommand> {
        vec![
            CommandBuilder::new("synthesize")
                ...
                .description("Synthesize speech via CosyVoice 3 and return WAV as base64. Does NOT play on host audio.")
                .param(
                ...
                .param(
                    optional_string("prompt_audio_path", "Reference Audio Path",
                        "Local wav path used for zero-shot voice cloning. Overrides `voice`."),
                )
                .param(
                    optional_string("prompt_text", "Reference Text",
                        "Transcript of `prompt_audio_path` (required for zero-shot cloning)."),
                )
                .param(
                    ParamBuilder::new("sample_mode", MetricDataType::String)
                        .display_name("Sampling Mode")
                        .description("Accepted for parity with moss-tts-nano; CosyVoice 3 ignores it.")
            ...
            CommandBuilder::new("speak")
                ...
                .param(
                    ParamBuilder::new("sample_mode", MetricDataType::String)
                        .display_name("Sampling Mode")
                        .description("Accepted for parity; CosyVoice 3 ignores it.")
                        .options(vec!["greedy".into(), "fixed".into(), "full".into()])
                        .default("greedy".into())
                        .build(),
                )
            ...
            CommandBuilder::new("list_voices")
                ...
                .description("List voice presets from the Python service.")
                .build(),
    ...
    async fn execute_command(&self, command: &str, args: &Value) -> Result<Value> {
        match command {
            "synthesize" => {
                ...
                let prompt_text = args.get("prompt_text").and_then(|v| v.as_str());
...
// CosyVoice 3 default SFT voices. The service may report more (or fewer) —
// use `list_voices` for the authoritative list. These are just UI hints.
...
fn voice_param() -> ParameterDefinition {
    ParamBuilder::new("voice", MetricDataType::String)
        .display_name("Voice")
        .description("Built-in voice preset. Overrides `COSYVOICE_VOICE` default.")
...
neomind_extension_sdk::neomind_export!(CosyVoiceExtension);
...
mod tests {
    ...
    #[test]
    fn metadata_is_sane() {
        let ext = CosyVoiceExtension::new();
        let m = ext.metadata();
        assert_eq!(m.id, "cosyvoice-3");
