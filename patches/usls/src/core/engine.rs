...
impl Engine {
    ...
    fn build_session(&mut self, inputs: &OrtTensorAttr) -> Result<Session> {
            ...
            Device::Cuda(id) => {
                ...
                #[cfg(feature = "cuda")]
                {
                    let ep = ort::execution_providers::CUDAExecutionProvider::default()
                        .with_device_id(id as i32)
                        // Limit GPU memory arena to 2 GiB so multiple models or other
                        // GPU processes can coexist on the same device.
                        .with_memory_limit(2 * 1024 * 1024 * 1024)
                        // Extend arena only by the requested amount instead of
                        // doubling (prevents aggressive pre-allocation).
                        .with_arena_extend_strategy(
                            ort::execution_providers::ArenaExtendStrategy::SameAsRequested
                        );
                    // Skip is_available() check — it can report false when the GPU ORT library
                    // is loaded dynamically and provider detection is unreliable. Just try to
                    // register directly; if it fails, the error will propagate naturally.
                    if let Err(err) = ep.register(&mut builder) {
                        anyhow::bail!(
                            "Failed to register CUDA EP (device {}): {}. {}",
                            id, err,
                            compile_help.replace("#EP", "CUDA")
                        );
                    }
                }
            }
        ...
        let session = builder
            ...
            // Disable ALL Gelu fusion variants: ORT's GeluFusion fuses
            // Erf+Add+Mul subgraphs into com.microsoft.Gelu (contrib op) which
            // the CUDA EP on Jetson lacks as a kernel → EP_FAIL. The fusion
            // runs at multiple levels with different names (GeluFusionL1,
            // GeluFusionL2, GeluFusion). Also disable BiasGeluFusion and
            // FastGeluFusion which create similar contrib ops.
