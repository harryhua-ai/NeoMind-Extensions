...
use crate::{
    ...
    Iiix, MinOptMax, ORTConfig, OnnxIo, Ops, OrtTensorAttr, PROGRESS_BAR_STYLE_CYAN_2,
...
impl OrtEngine {
    ...
    pub fn dry_run(&mut self) -> Result<()> {
        if self.num_dry_run > 0 {
            ...
            let create_tensor = |shape: Vec<usize>, dtype: &TensorElementType| -> Result<Tensor> {
                match dtype {
                    TensorElementType::Float32 => Tensor::ones::<f32>(shape),
                    TensorElementType::Float64 => Tensor::ones::<f64>(shape),
                    TensorElementType::Float16 => Tensor::ones::<half::f16>(shape),
                    TensorElementType::Bfloat16 => Tensor::ones::<half::bf16>(shape),
                    TensorElementType::Int8 => Tensor::ones::<i8>(shape),
                    TensorElementType::Int16 => Tensor::ones::<i16>(shape),
                    TensorElementType::Int32 => Tensor::ones::<i32>(shape),
                    TensorElementType::Int64 => Tensor::ones::<i64>(shape),
                    TensorElementType::Uint8 => Tensor::ones::<u8>(shape),
                    TensorElementType::Uint16 => Tensor::ones::<u16>(shape),
                    TensorElementType::Uint32 => Tensor::ones::<u32>(shape),
                    TensorElementType::Uint64 => Tensor::ones::<u64>(shape),
                    _ => Tensor::ones::<f32>(shape),
                }
            };

            // Create dummy tensors with appropriate types based on ONNX model info
            let tensors = self
                .inputs_minoptmax()
                .iter()
                .enumerate()
                .map(|(i, shape_info)| {
                    let shape: Vec<usize> = shape_info.iter().map(|dim| dim.opt()).collect();
                    let dtype = self
                        .onnx
                        .as_ref()
                        .map(|onnx_io| onnx_io.inputs.dtypes())
                        .and_then(|dt| dt.get(i))
                        .unwrap_or(&TensorElementType::Float32);
                    create_tensor(shape, dtype)
                })
    ...
    fn preprocess(tensor: &Tensor, dtype: &TensorElementType) -> Result<DynValue> {
        let x = match dtype {
            TensorElementType::Float32 => {
                if tensor.dtype() == slsl::DType::Fp32 {
                    tensor.clone()
                } else {
                    tensor.to_dtype::<f32>()?
                }
            ...
            TensorElementType::Float16 => {
                if tensor.dtype() == slsl::DType::Fp16 {
                    tensor.clone()
                } else {
                    tensor.to_dtype::<half::f16>()?
                }
            }
            TensorElementType::Bfloat16 => {
                if tensor.dtype() == slsl::DType::Bf16 {
                    tensor.clone()
                } else {
                    tensor.to_dtype::<half::bf16>()?
                }
            }
            TensorElementType::Float64 => {
                if tensor.dtype() == slsl::DType::Fp64 {
                    tensor.clone()
                } else {
                    tensor.to_dtype::<f64>()?
                }
            ...
            TensorElementType::Int8 => {
                if tensor.dtype() == slsl::DType::Int8 {
                    tensor.clone()
                } else {
                    tensor.to_dtype::<i8>()?
                }
            ...
            TensorElementType::Int16 => {
                if tensor.dtype() == slsl::DType::Int16 {
                    tensor.clone()
                } else {
                    tensor.to_dtype::<i16>()?
                }
            ...
            TensorElementType::Int32 => {
                if tensor.dtype() == slsl::DType::Int32 {
                    tensor.clone()
                } else {
                    tensor.to_dtype::<i32>()?
                }
            ...
            TensorElementType::Int64 => {
                if tensor.dtype() == slsl::DType::Int64 {
                    tensor.clone()
                } else {
                    tensor.to_dtype::<i64>()?
                }
            ...
            TensorElementType::Uint8 => {
                if tensor.dtype() == slsl::DType::Uint8 {
                    tensor.clone()
                } else {
                    tensor.to_dtype::<u8>()?
                }
            ...
            TensorElementType::Uint16 => {
                if tensor.dtype() == slsl::DType::Uint16 {
                    tensor.clone()
                } else {
                    tensor.to_dtype::<u16>()?
                }
            ...
            TensorElementType::Uint32 => {
                if tensor.dtype() == slsl::DType::Uint32 {
                    tensor.clone()
                } else {
                    tensor.to_dtype::<u32>()?
                }
            ...
            TensorElementType::Uint64 => {
                if tensor.dtype() == slsl::DType::Uint64 {
                    tensor.clone()
                } else {
                    tensor.to_dtype::<u64>()?
                }
            ...
            _ => unimplemented!("Unsupported tensor type: {:?}", dtype),
        };

        Self::tensor_to_dynvalue(&x, dtype)
    }

    fn postprocess(x: &DynValue, dtype: &TensorElementType) -> Result<Tensor> {
        match dtype {
            TensorElementType::Float32 => {
                let (shape, data) = x.try_extract_tensor::<f32>()?;
                let shape: Vec<_> = shape.iter().map(|x| *x as usize).collect();

                Tensor::from_slice(data, shape)
            }
            TensorElementType::Float16 => {
                let (shape, data) = x.try_extract_tensor::<half::f16>()?;
                let shape: Vec<_> = shape.iter().map(|x| *x as usize).collect();
                Tensor::from_slice(data, shape)
            }
            TensorElementType::Bfloat16 => {
                let (shape, data) = x.try_extract_tensor::<half::bf16>()?;
                let shape: Vec<_> = shape.iter().map(|x| *x as usize).collect();
                Tensor::from_slice(data, shape)
            }
            TensorElementType::Float64 => {
                let (shape, data) = x.try_extract_tensor::<f64>()?;
                let shape: Vec<_> = shape.iter().map(|x| *x as usize).collect();
                Tensor::from_slice(data, shape)
            }
            TensorElementType::Int8 => {
                let (shape, data) = x.try_extract_tensor::<i8>()?;
                let shape: Vec<_> = shape.iter().map(|x| *x as usize).collect();
                Tensor::from_slice(data, shape)
            }
            TensorElementType::Int16 => {
                let (shape, data) = x.try_extract_tensor::<i16>()?;
                let shape: Vec<_> = shape.iter().map(|x| *x as usize).collect();
                Tensor::from_slice(data, shape)
            }
            TensorElementType::Int32 => {
                let (shape, data) = x.try_extract_tensor::<i32>()?;
                let shape: Vec<_> = shape.iter().map(|x| *x as usize).collect();
                Tensor::from_slice(data, shape)
            }
            TensorElementType::Int64 => {
                let (shape, data) = x.try_extract_tensor::<i64>()?;
                let shape: Vec<_> = shape.iter().map(|x| *x as usize).collect();
                Tensor::from_slice(data, shape)
            }
            TensorElementType::Uint64 => {
                let (shape, data) = x.try_extract_tensor::<u64>()?;
                let shape: Vec<_> = shape.iter().map(|x| *x as usize).collect();
                Tensor::from_slice(data, shape)
            }
            TensorElementType::Uint32 => {
                let (shape, data) = x.try_extract_tensor::<u32>()?;
                let shape: Vec<_> = shape.iter().map(|x| *x as usize).collect();
                Tensor::from_slice(data, shape)
            }
            TensorElementType::Uint16 => {
                let (shape, data) = x.try_extract_tensor::<u16>()?;
                let shape: Vec<_> = shape.iter().map(|x| *x as usize).collect();
                Tensor::from_slice(data, shape)
            }
            TensorElementType::Uint8 => {
                let (shape, data) = x.try_extract_tensor::<u8>()?;
                let shape: Vec<_> = shape.iter().map(|x| *x as usize).collect();
                Tensor::from_slice(data, shape)
            }
            TensorElementType::Bool => {
                let (shape, data) = x.try_extract_tensor::<bool>()?;
                let shape: Vec<_> = shape.iter().map(|x| *x as usize).collect();
                Tensor::from_slice(data, shape)
            }
            _ => anyhow::bail!("Unsupported ort tensor type: {:?}", dtype),
        }
    }

    fn tensor_to_dynvalue(tensor: &Tensor, dtype: &TensorElementType) -> Result<DynValue> {
        match dtype {
            TensorElementType::Float32 => {
                let data = tensor.to_flat_vec::<f32>()?;
                let shape = tensor.dims();
                Ok(Value::from_array((shape, data))?.into_dyn())
            }
            TensorElementType::Float16 => {
                let data = tensor.to_flat_vec::<f16>()?;
                let shape = tensor.dims();
                Ok(Value::from_array((shape, data))?.into_dyn())
            }
            TensorElementType::Bfloat16 => {
                let data = tensor.to_flat_vec::<bf16>()?;
                let shape = tensor.dims();
                Ok(Value::from_array((shape, data))?.into_dyn())
            }
            TensorElementType::Float64 => {
                let data = tensor.to_flat_vec::<f64>()?;
                let shape = tensor.dims();
                Ok(Value::from_array((shape, data))?.into_dyn())
            }
            TensorElementType::Int8 => {
                let data = tensor.to_flat_vec::<i8>()?;
                let shape = tensor.dims();
                Ok(Value::from_array((shape, data))?.into_dyn())
            }
            TensorElementType::Int16 => {
                let data = tensor.to_flat_vec::<i16>()?;
                let shape = tensor.dims();
                Ok(Value::from_array((shape, data))?.into_dyn())
            }
            TensorElementType::Int32 => {
                let data = tensor.to_flat_vec::<i32>()?;
                let shape = tensor.dims();
                Ok(Value::from_array((shape, data))?.into_dyn())
            }
            TensorElementType::Int64 => {
                let data = tensor.to_flat_vec::<i64>()?;
                let shape = tensor.dims();
                Ok(Value::from_array((shape, data))?.into_dyn())
            }
            TensorElementType::Uint8 => {
                let data = tensor.to_flat_vec::<u8>()?;
                let shape = tensor.dims();
                Ok(Value::from_array((shape, data))?.into_dyn())
            }
            TensorElementType::Uint16 => {
                let data = tensor.to_flat_vec::<u16>()?;
                let shape = tensor.dims();
                Ok(Value::from_array((shape, data))?.into_dyn())
            }
            TensorElementType::Uint32 => {
                let data = tensor.to_flat_vec::<u32>()?;
                let shape = tensor.dims();
                Ok(Value::from_array((shape, data))?.into_dyn())
            }
            TensorElementType::Uint64 => {
                let data = tensor.to_flat_vec::<u64>()?;
                let shape = tensor.dims();
                Ok(Value::from_array((shape, data))?.into_dyn())
            }
            TensorElementType::Bool => {
                let data = tensor.to_flat_vec::<u8>()?;
                let bool_data: Vec<bool> = data.iter().map(|&x| x != 0).collect();
                let shape = tensor.dims();
                Ok(Value::from_array((shape, bool_data))?.into_dyn())
            }
            _ => unimplemented!("Unsupported tensor type: {:?}", dtype),
        }
    }

    fn build_session(&mut self, _inputs: &OrtTensorAttr) -> Result<Session> {
        #[allow(unused_mut)]
        ...
        match self.device {
            Device::TensorRt(_id) => {
                ...
                #[cfg(feature = "tensorrt")]
                {
                    ...
                    for (i, name) in _inputs.names.iter().enumerate() {
                        if i != 0 {
                            spec_min.push(',');
                            spec_opt.push(',');
                            spec_max.push(',');
